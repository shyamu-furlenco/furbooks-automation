...
SELECT /*+ BROADCAST(m) */
    m.month_num,
    'Total pickup (Pickup date basis)'                                           AS component,
    6                                                                            AS sort_order,
...
FROM churn_pickups_base                                                          cp
    JOIN months                                                                      m
ON cp.pickup_date >= m.m_start
    AND cp.pickup_date <  m.m_end
...
SELECT /*+ BROADCAST(m) */
    m.month_num,
    'TTO (Total - Pickup date basis)'                                            AS component,
    9                                                                            AS sort_order,
    -COUNT(DISTINCT CONCAT(cp.entity_type, '::', cp.entity_id))                  AS items_count,
    -COUNT(DISTINCT cp.user_ids)                                                 AS cx_count,
    -SUM(cp.taxable_amount::FLOAT)                                               AS taxable_revenue
...
SELECT month_num, 'Plan transition - Positive' AS component, 16 AS sort_order,
...
SELECT month_num, 'Plan transition - Negative' AS component, 17 AS sort_order,
    COUNT(DISTINCT CONCAT(accountable_entity_type, '::', accountable_entity_id)) AS items_count,
    COUNT(DISTINCT user_id) AS cx_count, SUM(revenue_difference) AS taxable_revenue
FROM customer_plan_changes
WHERE previous_end_date IS NOT NULL AND previous_tenure IS NOT NULL AND current_tenure < previous_tenure
...
-- Includes plan transition revenue delta (tenure-driven discount change affects per-cycle amount).
...
SELECT
    ...
    'Current month MTP',
    'Penalty',
    'Plan transition - Positive',
    'Plan transition - Negative'

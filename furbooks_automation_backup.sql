...
-- Step A: Base RR filter — all recognitions per schedule (no dedup).
-- SETTLEMENT records included intentionally; tenure gate in customer_plan_transition
-- naturally filters to schedule boundaries without needing QUALIFY.
        , rr_schedule_base AS (
SELECT
    ...
    DATE(start_date)     AS rr_start_date,
...
-- Step B: Explode discount array (no JOINs allowed in same FROM as LATERAL VIEW).
...
-- start_date from RR (not schedule) — consistent with how all other components bucket into months.
-- Schedule join is kept only for tenure (DATEDIFF on schedule-level dates).
...
-- Excludes UPFRONT discounts (captured in plan_transition) to avoid double-counting.
-- Two-CTE pattern: Spark SQL prohibits JOIN in same FROM as LATERAL VIEW EXPLODE.
-- ----------------------------------------------------------------------------

-- Step 1: Explode all discounts at RR level (no JOINs here).
        , rr_all_discounts_exploded AS (
SELECT
    rr.accountable_entity_id,
    rr.accountable_entity_type,
    DATE(rr.start_date) AS start_date,
...
-- Step 2: Sum non-UPFRONT discounts per cycle.
-- UPFRONT discounts are handled by plan_transition; only other discounts flow here.
        , discount_per_cycle AS (
SELECT
    e.accountable_entity_id, e.accountable_entity_type,
    e.start_date, e.end_date,
    COALESCE(SUM(e.discount_amount::DECIMAL(10,2)), 0) AS total_discount_amount
FROM rr_all_discounts_exploded e
...
GROUP BY e.accountable_entity_id, e.accountable_entity_type, e.start_date, e.end_date
    )
...
-- Net discount change (UPFRONT discounts excluded globally in discount_per_cycle).

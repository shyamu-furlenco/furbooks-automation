...
-- Entity-type splits: avoid re-scanning rr_base 7 times
    , rr_item_attach AS (
    SELECT * FROM rr_base WHERE accountable_entity_type IN ('ITEM', 'ATTACHMENT')
    )
    , rr_penalty AS (
    SELECT * FROM rr_base WHERE accountable_entity_type = 'PENALTY'
    )
    , rr_vas AS (
    SELECT * FROM rr_base WHERE accountable_entity_type = 'VALUE_ADDED_SERVICE'
    )
...
SELECT
    1 AS month_num,
    DATE_TRUNC('month',:month1_start)::DATE                                                     AS m_start,
...
-- Swap unified: single pass for both SWAP_OUT and SWAP_IN
    , swap_unified AS (
SELECT
    sw.entity_id,
    sw.entity_type,
    ...
    rb.monetary_components_taxableAmount                                         AS taxable_amount,
    ROW_NUMBER() OVER (
    PARTITION BY sw.entity_id, sw.entity_type, sw.action
...
ON  se.id          = sw.entity_id
    ...
    LEFT JOIN rr_item_attach rb
    ON  rb.accountable_entity_id   = sw.entity_id
    AND rb.accountable_entity_type = sw.entity_type
    AND (
        (sw.action = 'SWAP_OUT' AND rb.rr_start_date <= DATE(sw.fulfillment_date + INTERVAL '330 minutes'))
        OR
        (sw.action = 'SWAP_IN'  AND rb.rr_start_date >= DATE(sw.fulfillment_date + INTERVAL '330 minutes'))
    )
    )
...
SELECT
    dr.*,
    gd.type                                                            AS discount_type
FROM discounts_raw dr
    LEFT JOIN furlenco_silver.godfather_evolve.discounts gd ON dr.catalogReferenceId = gd.id
    )
...
WHERE e.discount_type = 'UPFRONT' OR e.external_reference_type = 'SETTLEMENT'
GROUP BY
...
-- Discount Change: non-UPFRONT discounts (from same explode CTE, no extra JOIN).
...
FROM discounts_exploded e
WHERE (e.discount_type IS NULL OR e.discount_type <> 'UPFRONT')
...
SELECT
    ...
    LAG(total_discount_amount) OVER w AS previous_discount_amount,
    LAG(start_date)            OVER w AS previous_start_date,
    LAG(end_date)              OVER w AS previous_end_date
FROM discount_per_cycle
    WINDOW w AS (PARTITION BY accountable_entity_id, accountable_entity_type ORDER BY start_date)
    )
...
-- Acquisition (merged — single pass)
UNION ALL SELECT month_num, 'New deliveries (Addition of Cx)', 4, nd_items, nd_cx, nd_rev FROM acquisition_components
UNION ALL SELECT month_num, 'Upsell (Addition in item count)', 5, us_items, us_cx, us_rev FROM acquisition_components
...
GROUP BY component, sort_order
        ...
        , opening_row AS (
SELECT
    COALESCE(m1_items, 0) AS m1_items,
    COALESCE(m1_cx,    0) AS m1_cx,
    COALESCE(m1_rev,   0) AS m1_rev,
    COALESCE(m2_items, 0) AS m2_items,
    COALESCE(m2_cx,    0) AS m2_cx,
    COALESCE(m2_rev,   0) AS m2_rev
FROM base_wide

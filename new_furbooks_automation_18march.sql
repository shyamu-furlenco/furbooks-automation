...
-- Important Notes: Currently, Swap In Items are not coming in fb_automation.
...
SELECT
    wc.component,
    m1.m_label                        AS `Month 1`,
    wc.m1_items                       AS `M1 Items count`,
    wc.m1_cx                          AS `M1 Cx count`,
    ROUND(wc.m1_rev, 2)               AS `M1 Taxable revenue (without VAS)`,
    m2.m_label                        AS `Month 2`,
    wc.m2_items                       AS `M2 Items count`,
    wc.m2_cx                          AS `M2 Cx count`,
    ROUND(wc.m2_rev, 2)               AS `M2 Taxable revenue (without VAS)`
FROM with_calculated wc
CROSS JOIN (SELECT m_label FROM months WHERE month_num = 1) m1
CROSS JOIN (SELECT m_label FROM months WHERE month_num = 2) m2
